...
/**
 * list_l - This function determines length of linked list
...
/**
 * list_to_str - This function returns an array of strings of the list->str
...
char **list_to_str(list_t *head)
{
	...
	if (!strs)
		return (NULL);
	for (i = 0; node_ptr; node_ptr = node_ptr->next, i++)
	{
		str = malloc(_strl(node_ptr->str) + 1);
		if (!str)
		{
			for (j = 0; j < i; j++)
				free(strs[j]);
 ...
 *
 * Return: index of node or -1 if empty
 */
